...
def parse_args(reference_dir):
    ...
    options.cmd_string = '%ssingularity run %sdocker://emn4/vifi python "scripts/run_vifi.py" %s' % (env_string,
...
if __name__ == '__main__':
    ...
    if options.bamfile is None:
        ...
        os.system("bwa mem -t %d -M %s %s %s | samtools view -bS - > %s/%s.bam" % (
        options.cpus, options.reference, options.forward, options.reverse, options.output_dir, options.prefix))
